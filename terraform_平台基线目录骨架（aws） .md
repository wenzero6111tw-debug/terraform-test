...
# docs/（架构文档模板集）
> 目的：把**系统目标与约束（NFR）/ 决策记录（ADR）/ 拓扑图 / 数据流 / 信任边界 / 权衡表**全部文档化。可直接复制到仓库根目录的 `docs/`。

## 目录结构
```text
docs/
├─ README.md                      # 文档索引（入口）
├─ architecture-overview.md       # 架构总览一页纸
├─ nfr-checklist.md               # NFR 清单（评审表）
├─ tradeoffs.md                   # 权衡与风险清单（表格）
├─ exercise-runbook.md            # 演练与压测报告模板（与 examples 对应）
├─ adr/
│  └─ ADR-001-sample.md           # ADR 模板 + 示例
└─ diagrams/
   ├─ logical-topology.md         # 逻辑拓扑（Mermaid）
   ├─ deployment-network.md       # 部署/网络（Mermaid）
   └─ sequences-and-dfd.md        # 时序图 & 信任边界（Mermaid）
```
...
- [NFR 清单](./nfr-checklist.md)
- [权衡与风险清单](./tradeoffs.md)
- [演练与压测报告模板](./exercise-runbook.md)
- ADR 列表：`docs/adr/`（示例：[ADR-001](./adr/ADR-001-sample.md)）
- 图谱：`docs/diagrams/`（逻辑/部署/时序/信任边界）
```

## architecture-overview.md（一页纸）
```md
# 项目名称 — 架构总览（一页）
**版本/日期**：2025-09-26  **负责人**：__  **读者对象**：工程/产品/安全/运营

## 1. 业务目标（≤3 条）
- 例：订单峰值 2k RPS，全球 P95 ≤ 200ms
...
## 3. NFR 摘要
- SLA 99.9% ；RTO 60m / RPO 15m；最小权限 + 全量加密；单位成本口径：__
> 详见 [NFR 清单](./nfr-checklist.md)

## 4. 高层拓扑图
> 见 [逻辑拓扑](./diagrams/logical-topology.md)
...
## nfr-checklist.md（精简评审表）
```md
# NFR 清单（评审用）
## 可用性/可靠性
- [ ] SLA：__  RTO：__  RPO：__  灾备：Pilot/Warm/MultiSite
## 安全/合规
- [ ] 默认加密(KMS)；审计集中(CloudTrail/Config)；最小权限(SSO/IRSA)
## 性能/容量
- [ ] 峰值 QPS/并发：__；P95：__；缓存策略：__；弹性：__
## 可观测/运维
- [ ] 日志/指标/追踪最小集；SLO/错误预算；蓝绿/金丝雀/自动回滚阈值
## 成本/可持续
- [ ] 预算与告警：__；单位业务成本口径：__；节省计划/RI：__
**评审结论**：准入 / 限期整改项：__（Owner/截止）
```

## tradeoffs.md（权衡与风险表）
```md
# 权衡与风险清单
| 主题 | 选项 | 决策 | 主要权衡 | 风险/代价 | 缓解/指标 |
|---|---|---|---|---|---|
| B2B 接入 | PrivateLink vs Peering vs 公网+WAF | PrivateLink | 最小暴露 vs 成本 | 端点成本↑ | 预算告警/端点分组 |
| DR 策略 | Warm vs Multi‑Site | Warm | 成本 vs RTO/RPO | 切换复杂 | 每月演练/Runbook |
| 数据层 | 读副本 vs 缓存 | 缓存+只读 | 延迟 vs 一致性 | 过期风险 | 命中率≥95%告警 |
```

## exercise-runbook.md（演练与压测报告模板）
```md
# 演练与压测报告
**场景**：__（例：大促峰值 / 跨 AZ 故障 / 主从切换）  **环境**：dev/staging/prod；Region/AZ：__

## 指标口径
- SLO：可用性 _99.9%_；P95 延迟 _≤ 300ms_；错误率 _<0.1%_
- 可靠性：RTO：__；RPO：__；MTTR：__
- 成本：单位业务量成本 = 账单周期总成本 / 业务量
...
## 结论与改进
- 达成/未达成项；下一步行动（Owner/截止）
```

## adr/ADR-001-sample.md（模板 + 示例）
```md
# ADR-001：对 B2B 只读访问采用 PrivateLink
**状态**：Accepted
**背景/约束**：第三方需私网访问；合规要求最小暴露与集中审计；预算有限。
**选项**：
- A. PrivateLink：最小暴露；端点成本↑；需端点策略
- B. TGW+Peering：连通灵活；路由复杂；安全边界大
- C. 公网+WAF：成本低；暴露面大；审计与限流复杂
**决策**：选择 A（PrivateLink）。
**后果**：
- 正面：安全边界清晰；审计集中；跨账号权限可控
- 负面：端点成本；端点配额管理 → 以租户分组、预算告警缓解
**验证**：连接演练报告链接；端点策略与可达性测试记录
```

## diagrams/logical-topology.md（逻辑拓扑）
```md
# 逻辑拓扑
```mermaid
graph LR
  User((User)) -->|HTTPS| API[API Gateway / ALB]
  API --> SVC[Order Service (ECS/EKS)]
  SVC --> DB[(Aurora/RDS)]
  SVC --> Cache[(ElastiCache)]
  SVC --> MQ[[SQS/SNS]]
  SVC --> Obj[(S3)]
  Partner[Partner VPC] -- PrivateLink --> SVC
```
```

## diagrams/deployment-network.md（部署/网络）
```md
# 部署/网络视图
```mermaid
flowchart TB
  subgraph VPC[Prod VPC 10.10.0.0/16]
    subgraph AZa[AZ-a]
      ALBa[ALB]:::pub --> TSa[Task xN]
    end
    subgraph AZc[AZ-c]
      ALBc[ALB]:::pub --> TSc[Task xN]
    end
    TSa -->|read/write| Aurora[(Aurora Multi-AZ)]
    TSc --> Aurora
    TSa --> Cache[(Redis)] & S3[(S3)]
  end
classDef pub fill:#eef,stroke:#99f;
```
```

## diagrams/sequences-and-dfd.md（时序 & 信任边界）
```md
# 时序 & 信任边界
**下单时序**
```mermaid
...
```mermaid
flowchart LR
  subgraph Internet[边界A：公网]
    U((User))
  end
  subgraph CorpVPC[边界B：公司VPC]
    ALB[(ALB)]
    S1[Order Svc]
    DB[(Aurora)]
    Logs[(CloudWatch Logs)]
  end
  subgraph Partner[边界C：合作方VPC]
    PartnerApp[Partner App]
  end
  U-->ALB-->S1-->DB
  PartnerApp-- PrivateLink -->S1
  S1-->Logs
```
```
...
# docs/ 示例填充（电商题材 · 可直接复制覆盖）
> 场景：**电商订单与库存**（读多写少、峰谷明显），目标在东京区部署一套最小而可验证的架构与文档。

## architecture-overview.md（示例）
```md
# 电商订单系统 — 架构总览（一页）
**版本/日期**：v0.1 / 2025-09-26  
**负责人**：你名  
**读者对象**：工程/产品/安全/运营

## 1. 业务目标（≤3）
- 大促峰值 2k RPS，日均 200 RPS，P95 ≤ 300ms
- 订单可用性 SLA ≥ 99.9%，支付链路观测齐全
- 单位订单成本 ≤ ¥0.30/千请求（示例口径）

## 2. 范围与边界（In/Out）
- In：下单/查询、库存扣减、支付回调、对 B2B 的只读查询
- Out：仓内 WMS、第三方支付内部实现、推荐/搜索

## 3. NFR 摘要
- 可靠性：RTO 60m / RPO 15m；跨 AZ，多区冷备不启
- 安全：KMS 全量加密；CloudTrail/Config 集中；最小权限（IRSA/权限边界）
- 成本：Budgets 月上限 ¥1500；单位订单成本口径见 NFR 文档
> 详见 [NFR 清单](./nfr-checklist.md)
...
## 5. 关键决策（链接 ADR）
- [ADR-001 多账号着陆区与 SCP 十条红线](./adr/ADR-001-multi-account.md)
- [ADR-002 B2B 只读访问采用 PrivateLink](./adr/ADR-002-privatelink.md)

## 6. 风险与假设
- 假设：支付网关可用性 ≥ 99.95%
- 风险：大促流量突增 → 熔断+排队、只读降级；缓存击穿 → 预热/锁
```

## nfr-checklist.md（已填示例）
```md
# NFR 清单（电商）
## 可用性/可靠性
- [x] SLA：99.9%   RTO：60m   RPO：15m   灾备：Warm（跨 AZ 主备 + 跨区只读）
## 安全/合规
- [x] 默认加密：KMS（应用/日志/数据库/S3）；审计集中：CloudTrail/Config；最小权限：SSO 权限集 + IRSA
## 性能/容量
- [x] 峰值 2k RPS；P95 ≤ 300ms；缓存策略：多级（Edge/应用侧），TTL 60s 热点预热；弹性：ALB+ECS ASG、队列背压
## 可观测/运维
- [x] 结构化日志；RED/USE 指标；分布式追踪；SLO 与错误预算；蓝绿/金丝雀（失败自动回滚）
## 成本/可持续
- [x] 月预算 ¥1500 + 告警；单位订单成本口径：月账单 / 订单数；节省计划：Compute SP 1年 无预付
**评审结论**：准入；整改项：缓存命中率 <95% 时的退避方案（Owner：架构/后端，D+14）
```

## adr/ADR-001-multi-account.md（示例）
```md
# ADR-001：多账号着陆区与 SCP 十条红线
**状态**：Accepted
**背景/约束**：
- 合规：审计集中、密钥分域；
- 运维：按环境与职责隔离，最小 blast radius；
- 成本：FinOps 按团队/环境归集。
**选项**：
- A. 单账号多 OU/多角色（简单但边界弱）
- B. **多账号（Control Tower）+ OU + SCP（推荐）**
- C. 混合云统一管控（复杂度高，当前阶段不选）
**决策**：选 B。以 OU 划分 Sandbox/Workloads/Security/Shared；SCP 十条红线禁止关闭审计、禁止未加密 S3、禁止公网开放等。
**后果**：
- 正面：边界清晰、审计/成本归集易；
- 负面：跨账号权限与网络复杂度↑ → 以权限边界模板、PrivateLink/TGW 蓝图标准化缓解。
**验证**：新账号纳管 10 分钟完成审计/KMS/预算；SCP 生效覆盖 100%。
```

## adr/ADR-002-privatelink.md（示例）
```md
# ADR-002：B2B 只读访问采用 PrivateLink
**状态**：Accepted
**背景/约束**：第三方需私网只读，最小暴露，审计集中，带宽成本可控。
**选项**：
- A. **PrivateLink**（最小暴露；端点成本↑；精确端点策略）
- B. TGW + VPC Peering（路由与安全边界更大）
- C. 公网 + WAF（暴露面大，审计/限流复杂）
**决策**：选 A。合作方通过 Endpoint Service 接入；仅开放 GET/分页查询；端点策略限制到资源级。
**后果**：
- 正面：零公网暴露；权限与审计集中；
- 负面：端点数量/带宽成本 → 以租户分组、预算告警、速率限制缓解。
**验证**：可达性/权限演练报告；端点策略单元测试；压测命中率/限速监控。
```

## diagrams/logical-topology.md（替换为电商版）
```md
# 逻辑拓扑（电商）
```mermaid
graph LR
  User((User)) -->|HTTPS| ALB[ALB]
  ALB --> ORD[Order API (ECS Fargate)]
  ORD --> DB[(Aurora MySQL)]
  ORD --> Cache[(ElastiCache Redis)]
  ORD --> MQ[[SQS]]
  ORD --> Obj[(S3 receipts)]
  Partner[Partner VPC] -- PrivateLink --> ORD
```
```

## diagrams/deployment-network.md（替换为电商版）
```md
# 部署/网络视图（电商）
```mermaid
flowchart TB
  subgraph VPC[Prod VPC 10.10.0.0/16]
    subgraph AZa[AZ-a]
      ALBa[ALB]:::pub --> TSa[Order xN]
    end
    subgraph AZc[AZ-c]
      ALBc[ALB]:::pub --> TSc[Order xN]
    end
    TSa -->|read/write| Aurora[(Aurora Multi-AZ)]
    TSc --> Aurora
    TSa --> Cache[(Redis)]
...
## diagrams/sequences-and-dfd.md（替换为电商版）
```md
# 时序 & 信任边界（电商）
**下单时序**
```mermaid
sequenceDiagram
  ...
  participant S as Order API
  participant D as Aurora
  participant P as Payment
  U->>A: POST /orders
  A->>S: Forward
  S->>D: Create order (pending)
  S->>P: Create payment intent
  P-->>S: Callback success
...
## exercise-runbook.md（示例场景小结）
```md
# 演练与压测报告（电商 v0.1）
**场景**：大促峰值 + 缓存优化  
**环境**：staging（ap-northeast-1）

## 指标口径
- SLO：可用性 99.9%；P95 ≤ 300ms；错误率 <0.1%
- 可靠性：RTO 60m；RPO 15m
- 成本：单位订单成本 = 月账单 / 订单数

## 步骤
1) 基线：无缓存，记录 P95/吞吐/错误率；
2) 变更：启用 Redis 缓存，TTL60s，热点预热；
3) 压测：k6 50→200→0 VUs；
4) 结果：P95 从 420ms 降至 180ms，错误率从 0.4% → 0.08%；单位成本下降 18%；
5) 改进：命中率跌破 95% 告警，自动退避 + 预热任务。
```
